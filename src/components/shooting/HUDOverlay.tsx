    ...
    <div className="absolute inset-0 pointer-events-none z-20">
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-2">
        <div className="hud-border px-4 py-2 pointer-events-auto corner-brackets">
          <div className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Target</div>
        ...
        <div className="hud-border px-6 py-2 text-center">
          <div className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Timer</div>
          <div className="font-hud text-2xl text-white tabular-nums">{shotTimer.toFixed(2)}</div>
        </div>
        ...
        <div className="hud-border px-4 py-2 text-right corner-brackets">
          ...
          <div className="font-hud text-2xl text-tactical-green text-glow-green">
            {totalScore}
            <span className="text-sm text-slate-500">/{maxPossible}</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute left-4 top-20 bottom-16 w-48">
        <div className="hud-border h-full p-3 overflow-y-auto">
          <div className="text-[10px] text-slate-500 font-mono uppercase tracking-wider mb-2">Shot Log</div>
          {shots.length === 0 ? (
            <div className="text-xs text-slate-600 italic">Waiting for shots...</div>
          ) : (
            <div className="space-y-1">
              {shots.map((shot, i) => (
                <div key={shot.id} className="flex items-center justify-between text-xs py-1 border-b border-tactical-border/30">
                  <span className="text-slate-500 font-mono">#{String(i + 1).padStart(2, '0')}</span>
                  <span className="font-hud font-bold" style={{ color: `hsl(${(shot.score / 10) * 120}, 100%, 50%)` }}>
                    {shot.score}
                  </span>
                </div>
              ))}
      ...
      <div className="absolute right-4 top-20 w-44">
        <div className="hud-border p-3 space-y-3">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${irPosition ? 'bg-tactical-green shadow-neon-green' : 'bg-slate-600'}`} />
          ...
          <div>
            <div className="text-[10px] text-slate-500 font-mono uppercase mb-1">IR Level</div>
            <div className="h-1.5 bg-tactical-darker rounded-full overflow-hidden">
              <div
                className="h-full transition-all duration-100 rounded-full"
                style={{
                  width: `${Math.min(100, (brightness / 255) * 100)}%`,
                  background: brightness > 200 ? '#ff2d55' : brightness > 100 ? '#ffd600' : '#00f0ff',
                }}
              />
            </div>
          </div>
          ...
          <div>
            <div className="text-[10px] text-slate-500 font-mono uppercase mb-1">Series</div>
            <div className="flex gap-1">
              {Array.from({ length: shotsPerSeries }).map((_, i) => (
                <div
                  key={i}
                  className={`w-3 h-3 rounded-sm border ${
                    i < seriesShots ? 'bg-tactical-accent/50 border-tactical-accent' : 'border-tactical-border bg-transparent'
                  }`}
                />
              ))}
            </div>
      ...
      {/* Corner decorations */}
      <svg className="absolute top-2 left-2 w-8 h-8 text-tactical-accent/20" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1"><path d="M0 12V0h12" /></svg>
      <svg className="absolute top-2 right-2 w-8 h-8 text-tactical-accent/20" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1"><path d="M32 12V0H20" /></svg>
      <svg className="absolute bottom-2 left-2 w-8 h-8 text-tactical-accent/20" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1"><path d="M0 20v12h12" /></svg>
      <svg className="absolute bottom-2 right-2 w-8 h-8 text-tactical-accent/20" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1"><path d="M32 20v12H20" /></svg>
    </div>
  );
}
